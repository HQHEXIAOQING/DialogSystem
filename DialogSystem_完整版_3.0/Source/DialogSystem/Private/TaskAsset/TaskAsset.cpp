...
void UTaskAsset::UpdateTaskState()
{
	...
	for (auto Secondary:NodeInfo->Map_IdToSecondaryTaskArray)
	{
		...
		{
			IsResultFromSecondaryTask(Secondary.Value);
		}
		else
		{
			//判断是否可以触发
...
bool UTaskAsset::IsResultFromSecondaryTask(USecondaryTaskBase* SecondaryTask)
{
	//如果有结果就直接跳过这个分段任务。
	if(SecondaryTask->SecondaryTaskResult != ETaskResultType::None){return true;}
	//结果对象
	ETaskResultType resultType = ETaskResultType::None;
	//循环所有结果，判断是否有结果
	for (auto ConditionResult:SecondaryTask->Map_SecondaryTaskTypeToCondition)
	{
		resultType = ConditionResult.Key;
		//判断结果是否达成
